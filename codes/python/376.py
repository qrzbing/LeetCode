...
class Solution:
    def wiggleMaxLength(self, nums: List[int]) -> int:
        l = len(nums)
        dp = [[1] * l for _ in range(2)]
        for i in range(1, l):
            if nums[i] > nums[i-1]:
                dp[0][i]=dp[1][i-1]+1   # up = down+1
                dp[1][i]=dp[1][i-1]     # down = down
            elif nums[i] < nums[i-1]:
                dp[1][i]=dp[0][i-1]+1   # down = up+1
                dp[0][i]=dp[0][i-1]     # up = up
            # print(dp)
            else:
                dp[0][i], dp[1][i] = dp[0][i-1], dp[1][i-1]
        return 0 if l == 0 else max(dp[0][l-1], dp[1][l-1])
    # def wiggleMaxLength(self, nums: List[int]) -> int:
    #     down = 1; up = 1
    ...
    #         elif nums[i] < nums[i-1]:
    #             down = up+1
    #     return 0 if len(nums) == 0 else max(up, down)
